...
module.exports = class ResourceCommand extends vars.Commando.Command {
	...
	run(msg, { mention }) {
		...
		temp +=
			`${
				msg.mentions.members.first()
					? msg.mentions.members.first().toString() + " has "
					: "You have "
			}` +
			vars.formatMass(vars.countries[id].resource) +
			" resource\n";
		var total = 0;
		for (x in vars.map) {
			for (y in vars.map[x]) {
		...
		var cMilitaryPop = Math.round(
			vars.countries[id].population.size *
				vars.countries[id].population.manpower /
				100
		);
		var armedPercent = 1;
		...
		armedPercent =
			vars.countries[id].resource /
			(cMilitaryPop * vars.countries[id].gun.cost);
		if (armedPercent > 1) {
			armedPercent = 1;
		}
		var cost =
			Math.round(vars.countries[id].gun.cost * cMilitaryPop) * armedPercent;
		var profit = total - cost;
		temp +=
			"you mine " + vars.formatMass(Math.round(total)) + " resource per turn\n";
		temp +=
			"and spend " +
			vars.formatMass(Math.round(cost)) +
			" per turn on weapons\n";
		temp +=
			"leaving you with " + vars.formatMass(Math.round(profit)) + " per turn\n";
