	...
	try {
		...
		if(c == undefined){
			...
			if(content[0] == call+"setgun"){
				if(content[1] != null){
					if(guns[msg.content.split(" ")[1]] != undefined){
						msg.channel.send("gun set!");
						countries[id].gun = guns[msg.content.split(" ")[1]];
					}else{
			...
			if(content[0] == call+"force"){
				...
				msg.channel.send("Your army is armed with "+countries[id].gun.name+" and you can give "+(armedPercent*100)+"% of your "+(cMilitaryPop*100)+" troops, this gun");
			}
